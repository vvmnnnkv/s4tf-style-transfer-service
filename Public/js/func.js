...
function submitCanvas(canvas, url, type, cb) {
    let base64 = dataURIToBase64(canvas.toDataURL(type));
    let request = { instances: [ {b64: base64 } ] };
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            ...
            cb(null, base64ToDataURI(res.predictions[0].b64, 'image/png'));
        } else {
    ...
    xhr.onerror = function() {
        cb(new Error("Oops! Something failed, please re-try"));
    };
...
function base64ToDataURI(base64, mediaType) {
